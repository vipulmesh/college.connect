...
async function handleEventSubmit(e) {
  ...
  showToast("Enhancing Event", "Using AI to improve description...");

  try {
    const res = await fetch("http://127.0.0.1:5000/ai/enhance-event", {
      method: "POST",
      headers: {
    ...
    if (data.enhancedDescription) {
      formData.description = data.enhancedDescription;
    }
    ...
    showToast("Event Created!", "AI-enhanced event submitted for approval 🚀");

  } catch (error) {
    console.error(error);
    showToast("AI Error", "AI failed, submitting original description.", "error");
    addEvent(formData);
    renderClubEvents();
  }
}
...
async function enhanceDescriptionWithAI() {
  ...
  if (!text.trim()) {
    showToast("No Description", "Please write a description first", "error");
    return;
  }

  showToast("Enhancing...", "AI is improving your description ✨");

  try {
    const response = await fetch("http://127.0.0.1:5000/ai/enhance-event", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (data.enhancedDescription) {
      textarea.value = data.enhancedDescription;
    }

    showToast("Done!", "Description enhanced successfully 🚀");

  } catch (error) {
    console.error(error);
    showToast("Error", "AI service not reachable", "error");
  }
}
